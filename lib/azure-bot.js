...
export async function createBot({ botName, resourceGroup, tenantId }) {
  ...
  // Enable Teams channel — fire concurrently since it's non-fatal and
  // subsequent steps (tunnel, config, app package) don't depend on it
  const teamsChannelPromise = (async () => {
  ...
  saveState(result);
  result.teamsChannelPromise = teamsChannelPromise;
  return result;
}
